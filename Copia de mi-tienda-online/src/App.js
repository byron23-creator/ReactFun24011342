...
// Componente que contiene la lógica principal para poder usar hooks de router
function AppContent() {
  const navigate = useNavigate();
  const [cartItems, setCartItems] = useState([]);
  // El estado 'isLoggedIn' ahora controla la UI
  ...
  const handleLogout = () => {
    localStorage.removeItem('userId'); // Limpia el ID de usuario
    setIsLoggedIn(false); // Actualiza el estado para cambiar la UI
    navigate('/'); // Redirige al login
  };

  // --- FUNCIÓN PARA ACTUALIZAR LA UI AL HACER LOGIN ---
